...
void show(cv::Mat& in)
{
    std::cout << in.size() << " " << cv::sum(in)[0] << std::endl;
}
...
template <typename T>
void pre_calc_for_bilinear_interpolate(const int height, const int width, const int pooled_height, const int pooled_width, const int iy_upper, const int ix_upper, T roi_start_h, T roi_start_w, T bin_size_h, T bin_size_w, int roi_bin_grid_h, int roi_bin_grid_w, std::vector<PreCalc<T>>& pre_calc) {
...
std::vector<std::vector<float>> _forward_box(const std::vector<ncnn::Mat>& features, std::vector<std::vector<float>>& proposals)
{
    ...
    ncnn::Net box_head;
    box_head.load_param("assert/box_head-sim-opt-opt.param");
    box_head.load_model("assert/box_head-sim-opt-opt.bin");
    ...
    box_predictor.load_param("assert/box_predictor-sim-opt.param");
    box_predictor.load_model("assert/box_predictor-sim-opt.bin");
...
{
    ncnn::Net decoder;
    decoder.load_param("assert/decoder-sim-opt.param");
    decoder.load_model("assert/decoder-sim-opt.bin");
    ...
    densepose_head.load_param("assert/densepose_head-sim-opt.param");
    densepose_head.load_model("assert/densepose_head-sim-opt.bin");
    ...
    densepose_predictor.load_param("assert/densepose_predictor-sim-opt.param");
    densepose_predictor.load_model("assert/densepose_predictor-sim-opt.bin");
...
int main()
{
    ...
    cv::Mat image = cv::imread("assert/01_7_additional.jpg", CV_LOAD_IMAGE_COLOR);
    ...
    // ************************************************ backbone
    /*
    * ncnn::Mat feature: [ p2:(256,200,200), p3:(256,100,100), p4:(256,50,50), p5:(256,25,25), p6:(256,13,13) ]
    */
    ncnn::Net backbone;
    backbone.load_param("assert/backbone-sim-opt.param");
    backbone.load_model("assert/backbone-sim-opt.bin");
    ...
    // anchor_generator
    /*
    * ncnn::Mat anchors: [ (1,120000,4), (1,30000,4), (1,7500,4), (1,1875,4), (1,507,4) ]
    */
    int anchors_size[5] = { 120000,30000,7500,1875,507 };
    std::vector<ncnn::Mat> anchors(5);
    for (int i = 0; i < 5; i++) {
        anchors[i] = ncnn::Mat(4, anchors_size[i], 1);
        std::ifstream in("assert/anchors_"+std::to_string(i)+".data", std::ios::in | std::ios::binary);
        in.read((char*)anchors[i].channel(0), anchors_size[i] * 4 * 4);
    }

    // rpn_head
    /*
    * ncnn::Mat pred_objectness_logits: [ (1,1,120000), (1,1,30000), (1,1,7500), (1,1,1875), (1,1,507) ]
    * ncnn::Mat pred_anchor_deltas: [ (1,120000,4), (1,30000,4), (1,7500,4), (1,1875,4), (1,507,4) ]
    */
    ncnn::Net rpn_head;
    rpn_head.load_param("assert/rpn_head-sim-opt.param");
    rpn_head.load_model("assert/rpn_head-sim-opt.bin");
    ...
    // decode_proposals
    /*
    * cv::Mat pred_proposals: [ (1,120000,4), (1,30000,4), (1,7500,4), (1,1875,4), (1,507,4) ]
    */
    ...
    /*
    * pred_instances : [N × 5] : ( x1, y1, x2, y2, score )
    * results : vector<ncnn::Mat>(coarse_segm,fine_segm,u,v,{x1,y1,x2,y2,score}) )
    */
