...
CTNet::CTNet()
{
    feature_img_extractor.load_param("assert/CT-Net/feature_img_extractor-sim-opt.param");
    feature_img_extractor.load_model("assert/CT-Net/feature_img_extractor-sim-opt.bin");

    feature_ref_extractor.load_param("assert/CT-Net/feature_ref_extractor-sim-opt-opt.param");
    feature_ref_extractor.load_model("assert/CT-Net/feature_ref_extractor-sim-opt-opt.bin");

    process1.load_param("assert/CT-Net/process1-sim-opt.param");
    process1.load_model("assert/CT-Net/process1-sim-opt.bin");

    regression.load_param("assert/CT-Net/regression-sim-opt.param");
    regression.load_model("assert/CT-Net/regression-sim-opt.bin");

    gridGen.load_param("assert/CT-Net/gridGen-sim-opt.param");
    gridGen.load_model("assert/CT-Net/gridGen-sim-opt.bin");

    netG1.load_param("assert/CT-Net/netG1-sim-opt-opt-opt.param");
    netG1.load_model("assert/CT-Net/netG1-sim-opt-opt-opt.bin");

    netG2.load_param("assert/CT-Net/netG2-sim-opt-opt-opt-opt.param");
    netG2.load_model("assert/CT-Net/netG2-sim-opt-opt-opt-opt.bin");
}
CTNet::~CTNet()
{
    feature_img_extractor.clear();
    feature_ref_extractor.clear();
    process1.clear();
    regression.clear();
    gridGen.clear();
    netG1.clear();
...
ncnn::Mat CTNet::get_label_tensor(cv::Mat candidate, cv::Mat subset, cv::Mat img)
{
...
cv::Mat CTNet::convert_seg(const cv::Mat& t_seg)
{
...
cv::Mat CTNet::convert_dp_mask(const cv::Mat& dp)
{
...
ncnn::Mat CTNet::mask_convert(const cv::Mat& seg, int c)
{
...
float CTNet::im2col_get_pixel(float* im, int height, int width, int channels, int row, int col, int channel, int pad)
{
    row -= pad;
    col -= pad;
...
ncnn::Mat CTNet::im2col_cpu(ncnn::Mat& data_im, int ksize, int stride, int pad)
{
...
ncnn::Mat CTNet::cv2ncnn(const cv::Mat& in)
{
...
cv::Mat CTNet::ncnn2cv(const ncnn::Mat& in)
{
    int H = in.h, W = in.w;
    if (in.c == 1) {
...
void CTNet::col2im_add_pixel(float* im, int height, int width, int channels, int row, int col, int channel, int pad, float val)
{
    row -= pad;
    col -= pad;
...
ncnn::Mat CTNet::col2im_cpu(ncnn::Mat& data_col, int output_size, int ksize, int stride, int pad)
{
...
float CTNet::SAFE_GET(const ncnn::Mat& input, int x, int y, int c, int H, int W)
{
    if (x >= 0 && x < W && y >= 0 && y < H)
...
ncnn::Mat CTNet::grid_sample(ncnn::Mat& input, ncnn::Mat& grid)
{
    int C = input.c;
    int IH = input.h;
    int IW = input.w;
...
cv::Mat CTNet::argmaxC(const ncnn::Mat& in)
{
...
std::vector<cv::Mat> CTNet::ncnn2cvC1(ncnn::Mat in)
{
    int C = in.c, H = in.h, W = in.w;
...
cv::Mat CTNet::morpho(const cv::Mat& mask)
{
...
cv::Mat CTNet::get_average_color(cv::Mat mask, cv::Mat arms)
{
...
void CTNet::dataset(ncnn::Mat& input_semantics, ncnn::Mat& ref_semantics, cv::Mat& ref_image, cv::Mat& real_image, cv::Mat& seg_img, cv::Mat& ref_seg, cv::Mat& img_dp_mask,
    cv::Mat src_rgb, cv::Mat dst_rgb, cv::Mat src_candidate, cv::Mat src_subset, cv::Mat dst_candidate, cv::Mat dst_subset, cv::Mat src_seg, cv::Mat dst_seg, cv::Mat src_iuv)
{
    ...
    // dst
    cv::Mat ref_tensor = dst_rgb.clone();
    ref_tensor.convertTo(ref_tensor, CV_32FC3, 1.0 / 0.5 / 255.0, -1.0);

    ncnn::Mat label_ref_tensor;
    {
        cv::Mat candidate = dst_candidate.clone();
        cv::Mat subset = dst_subset.t();
        cv::Mat img; cv::cvtColor(dst_rgb, img, cv::COLOR_RGB2BGR);
        label_ref_tensor = get_label_tensor(candidate, subset, img);
    }

    cv::Mat ref_seg_tensor = dst_seg.clone();
...
cv::Mat CTNet::inference(ncnn::Mat input_semantics, ncnn::Mat ref_semantics, cv::Mat ref_image, cv::Mat real_image, cv::Mat seg_img, cv::Mat ref_seg, cv::Mat img_dp_mask)
{
    // process
...
cv::Mat CTNet::call(cv::Mat src_rgb, cv::Mat dst_rgb, 
                    cv::Mat src_candidate,cv::Mat src_subset,cv::Mat dst_candidate,cv::Mat dst_subset,
                    cv::Mat src_seg, cv::Mat dst_seg,
                    cv::Mat src_iuv)
{


    ncnn::Mat input_semantics, ref_semantics;
    cv::Mat ref_image, real_image, seg_img, ref_seg, img_dp_mask;

    dataset(input_semantics, ref_semantics, ref_image, real_image, seg_img, ref_seg, img_dp_mask,
            src_rgb, dst_rgb, src_candidate, src_subset, dst_candidate, dst_subset, src_seg, dst_seg, src_iuv); // 数据转换&处理
